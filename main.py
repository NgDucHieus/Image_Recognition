...
THRESHOLDS = {
    "roof_lines": 20,          # Số đường thẳng cho "Mái tôn"
    "ruler_long_lines": 5,     # Số đường thẳng dài cho "Ảnh đo đạc bằng thước thép"
    "ruler_pixels": 2000,      # Số pixel đỏ cho thước
    "green_pixels": 5000,      # Số pixel xanh lá cho "Đo độ cứng thép"
    "connection_lines": 10,    # Số đường thẳng ngắn cho "Ảnh chi tiết liên kết"
    "thickness_red_pixels": 5000  # Số pixel đỏ cho "Đo chiều dày thép"
}

# Classification functions
def is_roof(img):
    """Phân loại 'Mái tôn' dựa trên số lượng đường thẳng dài."""
...
def is_ruler_measurement(img):
    """Phân loại 'Ảnh đo đạc bằng thước thép' dựa trên đường thẳng dài và màu đỏ."""
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    edges = cv2.Canny(gray, 50, 150)
    lines = cv2.HoughLinesP(edges, 1, np.pi / 180, threshold=100, minLineLength=100, maxLineGap=10)
    num_long_lines = 0
...
def is_hardness_test(img):
    """Phân loại 'Đo độ cứng thép' dựa trên màu xanh lá cây."""
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
...
def is_connection_detail(img):
    """Phân loại 'Ảnh chi tiết liên kết' dựa trên nhiều đường thẳng ngắn."""
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    edges = cv2.Canny(gray, 50, 150)
...
def is_thickness_measurement(img):
    """Phân loại 'Đo chiều dày thép' dựa trên màu đỏ (dụng cụ đo)."""
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    lower_red1 = np.array([0, 120, 70])
    upper_red1 = np.array([10, 255, 255])
    lower_red2 = np.array([170, 120, 70])
    ...
    mask_red = mask_red1 + mask_red2
    red_pixels = cv2.countNonZero(mask_red)
    return red_pixels > THRESHOLDS["thickness_red_pixels"]

def classify_image(image_path):
    """Phân loại ảnh và trả về nhãn cùng ảnh."""
    ...
    return "Ảnh tổng thể", img

# Main execution
if __name__ == "__main__":
    image_folder = os.path.abspath("Test")
    ...
    for image_file in image_files:
        ...
        if img is not None:
            ...
            # Corrected line: Generate a unique filename using uuid
            save_filename = f"{uuid.uuid4().hex}.jpg"  # Fixed syntax
    ...
    print("Notes:")
    print("- Adjust thresholds in THRESHOLDS for better accuracy (e.g., green_pixels).")
    print("- Install Pillow for fallback: `pip install Pillow`")
    print("- Run as admin if saving fails.")