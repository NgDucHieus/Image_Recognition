    ...
    def run(self):
        try:
            ...
            try:
                ...
                self.error.emit("Required packages not installed. Please install transformers and scikit-learn.")
                return
            ...
            for i, image_path in enumerate(image_files):
                ...
                try:
                    ...
                    if confidence >= confidence_threshold:
                        destination_path = os.path.join(self.destination_folder, pred_label, filename)
                        logging.info(f"Classified {filename} as '{pred_label}' (confidence: {confidence:.4f})")
                    else:
                        destination_path = os.path.join(self.destination_folder, "Skipped", filename)
                        logging.info(f"Skipped {filename}: low confidence ({confidence:.4f})")
                ...
                # Update progress
                progress_value = 40 + int((i + 1) / total_images * 55)  # 40%-95% range for processing
...
class ImageClassificationApp(QWidget):
    ...
    def on_classification_finished(self):
        ...
        QMessageBox.information(self, "Success", "Image classification completed successfully!")
